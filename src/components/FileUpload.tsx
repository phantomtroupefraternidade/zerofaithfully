...
// Exported so Library can reuse when loading from Supabase storage
export async function processBookFile(file: File): Promise<PageElement[][]> {
  const pages: PageElement[][] = [];
  const type = file.name.split('.').pop()?.toLowerCase() || '';

  const extractImagesFromZipStandalone = async (zip: JSZip): Promise<PageElement[]> => {
    const images: PageElement[] = [];
    const zipFiles = Object.keys(zip.files).filter(name => /\.(jpg|jpeg|png|webp|gif|bmp)$/i.test(name));
  ...
  if (type === 'pdf') {
    ...
    const images = await extractImagesFromZipStandalone(zip);
...
const FileUpload: React.FC<{ onFileProcessed: (fileData: FileData) => void }> = ({ onFileProcessed }) => {
  ...
  const processFile = async (file: File, category: string) => {
    try {
      const pages = await processBookFile(file);
      onFileProcessed({
        name: file.name,
        pages,
